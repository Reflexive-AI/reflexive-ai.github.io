...
**Capability Asymmetry:** As discussed in Research Object 006, auditors cannot match the understanding that developers have of their own models. A lab spending $100M on training will always know more than an auditor spending $100K on testing. The audit provides false assurance rather than genuine verification.

**Time Cost:** Comprehensive external audit takes time (weeks or months). In a fast-moving competitive environment, this delay is costly. Labs rush through audits, auditors cut corners to meet deadlines, and the resulting certification reflects schedule pressure rather than thorough evaluation.

**Regulatory Capture:** The dynamics that lead auditors to favor their clients (see Research Object 006) undermine the independence that makes external audit valuable. Over time, the market for auditors selects for those who are "easy" rather than those who are thorough.
...
Under pure external audit (regulator plays strategy 1), the lab must either invest in safety or risk failing the audit. But the lab also faces IP costs and delays, which drives it to avoid regulation entirely (jurisdictional arbitrage) or to capture the auditor.
...
Implementation challenges include: How are labs selected? (Random selection prevents gaming, but misses the highest-risk systems.) What triggers audit? (Risk-based triggers get gamed; random triggers waste resources on low-risk systems.) How are penalties structured? (Fines, market exclusion, criminal liability?)
...
**Attribution:** If a model contributes to harm, proving causation is difficult. Did the model provide the key information, or would the harm have occurred anyway?
...
**Long Tail:** A lab deploys a hundred models with minor issues before one causes catastrophic harm. Outcome-based enforcement might bankrupt the lab after the catastrophe, but that doesn't help the victims or undo the damage.
...
Active research addresses these limitations. As ZK-ML techniques mature, the feasibility of this approach will improve.
